...
class PlayState extends MusicBeatState
{
	...
	public var blitzBar:FlxBar;
	public var blitzBarBG:AttachedSprite;
	var songPercent:Float = 0;
	var blitzPercent:Float = 0;
	var multiplier:Int = 1;
	public static var OriginalScrollSpeed:Float;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				var stageFront:BGSprite = new BGSprite('stagefront', -760, 600, 0.9, 0.9);
				...
				wk1ppl = new BGSprite('BGStageppl', -200, 800, 1,1, ['BgBois']);
				wk1ppl.scale.set(1.9,1.9);
				wk1ppl.updateHitbox();
				add(wk1ppl);
		...
		blitzBarBG = new AttachedSprite('timeBar');
		blitzBarBG.screenCenter(X);
		blitzBarBG.screenCenter(Y);
		blitzBarBG.scrollFactor.set();
		blitzBarBG.alpha = 0;
		blitzBarBG.visible = !ClientPrefs.hideTime;
		blitzBarBG.color = FlxColor.BLACK;
		blitzBarBG.xAdd = -4;
		blitzBarBG.yAdd = -4;
		add(blitzBarBG);

		blitzBar = new FlxBar(blitzBarBG.x + 4, blitzBarBG.y + 4, LEFT_TO_RIGHT, Std.int(blitzBarBG.width - 8), Std.int(blitzBarBG.height - 8), this,
			'blitzPercent', 0, 100);
		blitzBar.scrollFactor.set();
		blitzBar.createFilledBar(0xFF000000, 0xFFFFFFFF);
		blitzBar.numDivisions = 800; //How much lag this causes?? Should i tone it down to idk, 400 or 200?
		blitzBar.alpha = 0;
		blitzBar.visible = !ClientPrefs.hideTime;
		add(blitzBar);
		blitzBarBG.sprTracker = blitzBar;
	...
	{
		OriginalScrollSpeed = SONG.speed;
		startingSong = false;
	...
	override public function update(elapsed:Float)
	{
		...
		if(dad.curCharacter == 'dad'){
			...
			if(boyfriend.animation.curAnim.name == 'hey' && IsGfReady == false){
	...
	{
		SONG.speed = OriginalScrollSpeed;
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				{
					boyfriend.animation.play('pre-attack');
					camFollowPos.x = boyfriend.x;
					camFollowPos.y = boyfriend.y;
					FlxTween.tween(camera, {'zoom': 3}, 0.3);
	...
	public function ResetScrollSpeed()
	{
		SONG.speed = OriginalScrollSpeed;
	}
}
